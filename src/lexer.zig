...
pub const Lexer = struct {
        ...
        if (self.peek() == '/') {
            ...
            while (self.peek() != 0) {
                if (self.get() == '\n') {
                    ...
                    const token = Token{ .kind = Kind.Comment, .lexeme = self.code[start - 2 .. self.cursor - 1], .line = self.line, .column = self.column - size - 2 };
                    self.line += 1;
                    self.column = 0;
                    return token;
                }
            ...
            const size: u16 = @intCast(self.cursor - start);
            return Token{ .kind = Kind.Unexpected, .lexeme = self.code[self.cursor .. self.cursor + 1], .line = self.line, .column = self.column - size };
        }
        const size: u16 = @intCast(self.cursor - start);
        return Token{ .kind = Kind.Slash, .lexeme = self.code[start..self.cursor], .line = self.line, .column = self.column - size };
    }
    ...
    fn identifier(self: *Lexer) Token {
        ...
        for (KeywordsCXX) |keyword| {
            if (std.mem.eql(u8, keyword, lexeme))
                return Token{ .kind = Kind.Keyword, .lexeme = lexeme, .line = self.line, .column = self.column - size };
        }
        return Token{ .kind = Kind.Identifier, .lexeme = lexeme, .line = self.line, .column = self.column - size };
    }
    ...
    fn number(self: *Lexer) Token {
        ...
        const size: u16 = @intCast(self.cursor - start);
        return Token{ .kind = Kind.Number, .lexeme = self.code[start..self.cursor], .line = self.line, .column = self.column - size };
    }
    ...
    fn atom(self: *Lexer, kind: Kind) Token {
        ...
        return Token{ .kind = kind, .lexeme = self.code[self.cursor - 1 .. self.cursor], .line = self.line, .column = self.column - 1 };
    }

    pub fn next(self: *Lexer) Token {
        if (self.cursor == self.code.len)
            return Token{ .kind = Kind.End, .lexeme = &[_]u8{0}, .line = self.line, .column = 0 };
        ...
        return switch (self.peek()) {
            0 => Token{ .kind = Kind.End, .lexeme = &[_]u8{0}, .line = self.line, .column = self.column },
